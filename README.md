...
This project asks where the thermodynamic errors of uMLIPs come from. It computes harmonic phonons by the finite-displacement method (phonopy), evaluates entropy, heat capacity and free energy from the phonon partition function, and compares them with the NIST-JANAF tables for nine crystals. The error is then taken apart in two ways: pairs of models that differ mainly in one factor partially disentangle the effects of the training data, the exchange–correlation functional and the architecture (other details, such as model size and training settings, may also differ within a pair); and recomputing the phonons at the experimental static-lattice constant separates an error in the lattice constant from an error in the force constants themselves.
...
The models form controlled pairs: MACE-MP-0 and MACE-OMAT-0 differ mainly in the training data, the two MatPES models mainly in the DFT functional (and slightly in the number of structures, about 430k PBE vs 390k r2SCAN), MACE-OMAT-0 and ORB-v3-OMAT mainly in the architecture, and MACE-MP-0 with and without D3 only in the dispersion correction. Models whose dependencies conflict with MACE (MatterSim and SevenNet require e3nn ≥ 0.5, MACE pins 0.4.4) will run the `compute` step in a separate environment and pass on `phonopy_params.yaml`.
